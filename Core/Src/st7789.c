...
{
	HAL_GPIO_WritePin(RES_TFT_GPIO_Port, RES_TFT_Pin, GPIO_PIN_RESET);
	HAL_Delay(10);
  HAL_GPIO_WritePin(RES_TFT_GPIO_Port, RES_TFT_Pin, GPIO_PIN_SET);
  HAL_Delay(150);
}
...
{
  HAL_GPIO_WritePin(DC_TFT_GPIO_Port, DC_TFT_Pin, GPIO_PIN_RESET);
  //HAL_GPIO_WritePin(CS_GPIO_Port, CS_Pin, GPIO_PIN_RESET);
	if (READ_BIT(SPI2->CR1, SPI_CR1_SPE) != (SPI_CR1_SPE)) SET_BIT(SPI2->CR1, SPI_CR1_SPE);
	*((__IO uint8_t *)&SPI2->DR) = Cmd;
  //HAL_GPIO_WritePin(CS_GPIO_Port, CS_Pin, GPIO_PIN_SET);
}

void ST7789_SendData(uint8_t Data)
{
	HAL_GPIO_WritePin(DC_TFT_GPIO_Port, DC_TFT_Pin, GPIO_PIN_SET);
  //HAL_GPIO_WritePin(CS_GPIO_Port, CS_Pin, GPIO_PIN_RESET);
  if (READ_BIT(SPI2->CR1, SPI_CR1_SPE) != (SPI_CR1_SPE)) SET_BIT(SPI2->CR1, SPI_CR1_SPE);
...
#else
  if (Value)
		HAL_GPIO_WritePin(BLK_TFT_GPIO_Port, BLK_TFT_Pin, GPIO_PIN_SET);
  else
		HAL_GPIO_WritePin(BLK_TFT_GPIO_Port, BLK_TFT_Pin, GPIO_PIN_RESET);
#endif
}
